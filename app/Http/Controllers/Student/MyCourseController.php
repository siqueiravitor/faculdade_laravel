...
class MyCourseController extends Controller
{
    ...
    public function myLearningCourseList(Request $request)
    {
        ...
        $data['enrollments'] = Enrollment::where('enrollments.user_id', auth()->id())->select('enrollments.*', 'order_items.unit_price')->join('orders', 'orders.id', '=', 'enrollments.order_id')->join('order_items', 'order_items.order_id', '=', 'orders.id')->where('enrollments.status', ACCESS_PERIOD_ACTIVE)->join('courses', 'courses.id', '=', 'enrollments.course_id')->where('courses.status', ACCESS_PERIOD_ACTIVE)->groupBy('enrollments.id');
    ...
    public function primeiroLogin()
    {
        $data['pageTitle'] = 'My Learning Courses';
        $user = Auth::id();
        $data['dados'] = User::join('students', 'users.id', '=', 'students.user_id')
            ->where('users.id', '=', $user)
            ->first();
        $idcurso =  $data['dados']->curso;

        if (!$data['dados'] || !$data['dados']->curso) {
            $data['validacao']  =  0;
            return view('frontend.student.aceite-contrato', $data);
        }

        $data['curso'] = Category::find($idcurso);
        // dd($data['curso']);

        $data['camposcursos'] = Contrato::join('contrato_curso', 'contratos.id', '=', 'contrato_curso.contrato_id')
            ->leftjoin('campos_contrato', 'contratos.id', '=', 'campos_contrato.contrato_id')
            ->where('contrato_curso.category_id', '=', $idcurso)
            ->get();

        // dd($data['dados']->course_id);

        return view('frontend.student.aceite-contrato', $data);
    }

    public function aceitacontrato(Request $request)
    {
        $student = Auth::user()->student; // Obtém o modelo Student associado ao usuário autenticado

        // Verifica se o checkbox foi marcado
        if ($request->has('aceitarContrato')) {
            // Lógica para aceitar o contrato
            $student->accepted_at = now();
            $student->primeiro_login = 2;
            $student->save();

            return redirect()->route('student.my-learning')->with('success', 'Contrato aceito com sucesso.');
        } else {
            // Checkbox não marcado, talvez exiba uma mensagem de erro
            return redirect()->back()->with('error', 'Você deve aceitar os termos do contrato.');
        }
    }

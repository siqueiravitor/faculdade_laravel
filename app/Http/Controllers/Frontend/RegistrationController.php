...
class RegistrationController extends Controller
{
    ...
    public function storeSignUp(SignUpRequest $request)
    {
        ...
        if(get_option('registration_system_bonus_mode', 0)){
            ...
            createTransaction($user->id, $balance, TRANSACTION_REGISTRATION_BONUS, 'Bônus de registro');
        }
        ...
        if (get_option('registration_email_verification') == 1){
            try {
                ...
                toastrMessage('error', 'Algo está errado. Tente depois de alguns minutos!');
                return redirect()->back();
            }
            $this->showToastrMessage('error', __('Enviamos um e-mail de verificação para sua conta. Por favor verifique seu email.'));
        }
        $this->showToastrMessage('success', __('Seu cadastro foi realizado com sucesso.'));
    ...
    public function emailVerification($token)
    {
        if (User::where('remember_token', $token)->count() > 0)
        {
            ...
            $this->showToastrMessage('success', __('Parabéns! Seu e-mail foi verificado com sucesso.'));

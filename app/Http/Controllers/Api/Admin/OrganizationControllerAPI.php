...
class OrganizationControllerAPI extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Validação para 'first_name'
        if (empty($_POST['first_name']) || strlen($_POST['first_name']) > 100) {
            $validationErrors['first_name'] = 'O campo first_name é obrigatório e deve ter no máximo 100 caracteres.';
        }

        // Validação para 'email'
        if (empty($_POST['email']) || !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) || strlen($_POST['email']) > 255) {
            $validationErrors['email'] = 'O campo email é obrigatório, deve ser um endereço de e-mail válido e ter no máximo 255 caracteres.';
        } else {
            // Verifica se o email já existe no banco de dados
            if (User::where('email', $_POST['email'])->exists()) {
                $validationErrors['email'] = 'Este endereço de e-mail já está em uso.';
            }
        }

        // Validação para 'password'
        if (empty($_POST['password']) || strlen($_POST['password']) < 6) {
            $validationErrors['password'] = 'O campo password é obrigatório e deve ter no mínimo 6 caracteres.';
        }

        // Validação para 'phone_number'
        if (!is_numeric($_POST['phone_number'])) {
            $validationErrors['phone_number'] = 'O campo phone_number deve ser numérico.';
        } else {
            // Verifica se o telefone já existe no banco de dados
            if (User::where('phone_number', $_POST['phone_number'])->exists()) {
                $validationErrors['phone_number'] = 'Este número de telefone já está em uso.';
            }
        }

        // Validação para 'address'
        if (empty($_POST['address'])) {
            $validationErrors['address'] = 'O campo address é obrigatório.';
        }

        // Validação para 'name' (first_name + last_name)
        $name = $_POST['first_name'] . ' ' . $_POST['last_name'];
        if (User::where('name', $name)->exists()) {
            $validationErrors['first_name'] = 'Este nome já está em uso.';
        }

        // Se houver erros de validação, retorne a resposta adequada
        ...
        try {
            ...
            $organization_data = [
                ...
                'about_me' => $request->about_me,
                'postal_code' => $request->postal_code,
                'social_link' => '{"facebook":null,"twitter":null,"linkedin":null,"pinterest":null}',
            ];
            ...
            $organizationId = Organization::latest('id')->first()->id;

            DB::commit();

            // Retornar resposta JSON com ID e mensagem de sucesso
            return response()->json([
                'id' => $organizationId,
            ...
            return response()->json([
                'error' => $e->getMessage(),
            ], 500); // ou o código de status HTTP apropriado para erro interno do servidor
        }
    }
    ...
    public function update(Request $request, $uuid)
    {
        try {
            ...
            return response()->json([
                // 'id' => $organizationId,
                'message' => __('Unidade atualizada com sucesso'),
            ], 200);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json([

...
class difficultyLevelController extends Controller
{
    ...
    public function store(CourseLanguageRequest $request)
    {
        ...
        $this->model->create($request->only($this->model->getModel()->fillable));
          ...
          $model = 'difficulty_level'; // ou qualquer valor que você tenha

        // Substituir 'Category' por 'categoria'
        $modelForMessage = str_replace('difficulty_level', 'Dificuldade', $model);

        // Agora, você pode usar $modelForMessage onde precisar
        return $this->controlRedirection($request, 'difficulty-level', $modelForMessage);

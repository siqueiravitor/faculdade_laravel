...
class LoginController extends Controller
{
    ...
    */
    ...
    {
        $request->validate([
            'cpf_matricula_email' => 'required',
            'password' => 'required',
        ]);

        $field = null;
        $inputValue = $request->input('cpf_matricula_email');

        if (strlen($inputValue) === 11 && is_numeric($inputValue)) {
            $field = 'cpf';
            ...
            // return response()->json(['message' => 'Forneça um CPF, Matrícula ou E-mail válidos'], 422);
            $this->showToastrMessage('error', __('Ops! You have entered invalid credentials'));
        return redirect("login");
        }

        $request->merge([$field => $request->input('cpf_matricula_email')]);
        ...
        */
        if (Auth::attempt($credentials)) {
            ...
            if (Auth::user()->is_admin()) {
                ...
                if ($student->primeiro_login == 1) {
                    // dd($student);
                    // Se for o primeiro login, redirecione para a página de aceite de contrato
                    return redirect(route('student.aceite-contrato'));
                } else {
                    // Se não for o primeiro login, redirecione conforme a lógica anterior

...
class AffiliateController extends Controller
{
    ...
    public function becomeAffiliateApply(Request $request)
    {
        ...
        if(!is_null($obj)){
            $this->showToastrMessage('error', __('Você já tem uma solicitação pendente'));
            return redirect()->back();
        }
        $obj = AffiliateRequest::whereUserId(Auth::id())->where(['status' => STATUS_APPROVED])->first();
        if(!is_null($obj)){
            $this->showToastrMessage('error', __('Você já tem uma solicitação aprovada'));
            return redirect()->back();
        }
        if (is_null($obj)) {
        ...
        $this->showToastrMessage('success', __('Solicitação criada com sucesso'));
        return redirect()->back();
    }
}

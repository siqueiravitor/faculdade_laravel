...
function getCredentials(){
    $credentials = '{"web":{"client_id":"","project_id":"stellar-stream-395523","auth_uri":"https://accounts.google.com/o/oauth2/auth","token_uri":"https://oauth2.googleapis.com/token","auth_provider_x509_cert_url":"https://www.googleapis.com/oauth2/v1/certs","client_secret":"","redirect_uris":[""]}}';
...
function studentCourseProgress($course_id, $enrollment_id)
{
    ...
    if ($course->course_type == COURSE_TYPE_GENERAL) {
        ...
        if ($course->scorm_course) {
            $enrollment = Enrollment::whereId($enrollment_id)->first();
            $result = ($enrollment) ? ($enrollment->completed_time / max(1, $course->scorm_course->duration_in_second)) * 100 : 0;
        } else {
            // Lida com o caso em que $course->scorm_course é nulo
            $result = 0;
        }
    }
